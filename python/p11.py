...
def right(i, j):
    return (int(grid[i+x][j]) for x in xrange(0, 4))

def down(i, j):
    return (int(grid[i][j+x]) for x in xrange(0, 4))

def diag_down(i, j):
    return (int(grid[i+x][j+x]) for x in xrange(0, 4))

def diag_up(i, j):
    return (int(grid[i+3-x][j+x]) for x in xrange(0, 4))

def prods_adjacents():
    for x in xrange(0, len(grid)-3):
        for y in xrange(0, len(grid[0])-3):
            for f in [right, down, diag_down, diag_up]:
                yield reduce(mul, f(x, y))

print max(prods_adjacents())
